...
%   result struct with paths, manifest, and per-channel waveform data
    ...
    try
        ...
        runCommandList(io, cfg.acquisition.postArmCommands);
...
function runCommandList(io, cmds)
    if isempty(cmds)
        return;
    end
    cmds = string(cmds(:));
...
function tryWriteLine(io, cmd)
    writeline(io.io, char(cmd));
end

function waitForAcquisitionComplete(io, timeoutSeconds)
    if nargin < 2 || isempty(timeoutSeconds)
        timeoutSeconds = 30;
    end

    oldTimeout = io.Timeout;
    c = onCleanup(@() setTimeout(io, oldTimeout));
