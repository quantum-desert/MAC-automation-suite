...
cfg.paths.s1_run_dir = ['/Users/agentatom/Library/CloudStorage/OneDrive-Umich/' ...
    'GraduateSchool/UM/QE_LAB/MAC/data/codex_processing/Advantage_Data/' ...
...
cfg.s2.pipeline.trim.mode = 'trim_end';
cfg.s2.pipeline.trim.points = 2592;
...
sweepr = filter_grid_search(lp1,hp1,out.s1,'s1',cfg,plot_opts);

% fine grid sweep
% locate ridge window from top percentile
prc = 10;
thr = prctile(SNRr(:), 100-prc);      % top prc%
[r,c] = find(SNRr >= thr);
...
SNRr_fine = nan(numel(lp2),numel(hp2));

for r_lp = 1:numel(lp2)
    % set LP cutoff
    cfg.s1.pipeline.filter.ratio = lp2(r_lp);

    for r_hp = 1:numel(hp2)
        % set HP cutoff
        cfg.s1.pipeline.filter.ratio_hp = hp2(r_hp);

        % run processing
        out.s1 = run_single_dataset(cfg.paths.s1_run_dir, cfg.s1, 'S1',verbose);
    
        % store SNRe
        SNRr_fine(r_lp,r_hp) = out.s1.snre;
    end
    
end

% visualize fine map
figure; imagesc(hp2, lp2, SNRr_fine); axis xy; hold on
xlabel('HP Ratio'); ylabel('LP Ratio'); colorbar
title('Fine scan near bright ridge')

% pick out max
[maxSnrFine, k] = max(SNRr_fine(:));
[iLP, iHP] = ind2sub(size(SNRr_fine), k);
bestLPfine = lp2(iLP);  % row axis value
bestHPfine = hp2(iHP);  % col axis value
plot(bestHPfine, bestLPfine, 'rx', 'MarkerSize', 12, 'LineWidth', 2);

% apply best filters
cfg.s1.pipeline.filter.ratio = bestLPfine;
cfg.s1.pipeline.filter.ratio_hp = bestHPfine;
out.s1 = run_single_dataset(cfg.paths.s1_run_dir, cfg.s1, 'S1',verbose);

% report
fprintf('\nFine scan best:\nLP ratio = %.5f, HP ratio = %.5f', ...
    bestLPfine, bestHPfine);
fprintf('\nSNRe = %.3f, Adv. (dB) = %.2f', ...
    maxSnrFine,out.s1.adv_db);

figure;ax=gca; grid off; theme light;
plot_fft_from_outsx(ax,out.s1,strcat("fc_lp=",num2str(round(bestLPfine,3)),";fc_hp=",num2str(round(bestHPfine,3))));
xlim([0 40]); ylim([-80 1]);
...
if(~independent)
    % TODO select best filter combo

% % select + apply best filter ratio
% [~,idx] = max(SNRr); best_ratio = filt_options_lp(idx);
% cfg.s1.pipeline.filter.ratio = best_ratio;
% 
% % run processing w/ best filter
% verbose=true;
% out.s1 = run_single_dataset(cfg.paths.s1_run_dir, cfg.s1, 'S1',verbose);
end
...
if(~independent)
% % select + apply best filter ratio
% [~,idx] = max(SNRr); best_N = filt_options(idx);
% cfg.s1.pipeline.filter.N = best_N;
% 
% % run processing w/ best filter
% verbose=true;
% out.s1 = run_single_dataset(cfg.paths.s1_run_dir, cfg.s1, 'S1',verbose);
...
% %%  manual filter sweep - boxcar
% % set flags
% verbose=false;
% % S1
% % sweep through boxcar filter for each channel
% filt_options = [1:cfg.s1.pipeline.M+3];
% 
% % set up plotting
% figure; hold on; ax=gca;
% theme light;
...
% 
% % set causal LP
% cfg.s1.pipeline.filter.mode = 'boxcar'; 
% 
% SNRr = zeros(size(filt_options));
% 
% for r = 1:numel(filt_options)
%     % set LP cutoff
%     cfg.s1.pipeline.filter.N = filt_options(r);
% 
%     % run processing
%     out.s1 = run_single_dataset(cfg.paths.s1_run_dir, cfg.s1, 'S1',verbose);
% 
...
% 
%     % plot fft
%     plot_fft_from_outsx(ax,out.s1,strcat('boxcar N=',num2str(cfg.s1.pipeline.filter.N)));
% 
%     drawnow;          % force UI refresh now
%     % pause(1);       % wait ~1 second before next call
% 
...
% options
window_options = [1:5*cfg.s1.pipeline.M];
...
figure;
hold on; theme light;
plot(window_options,SNRr);
yline(out.s1.snr_c,'--','DisplayName','SNRc');
xlabel('window size');
ylabel('SNRe');

if(~independent)
% select + apply best window
...
p.filter.mode = 'fft_lp_ratio';   % 'none' | 'fft_lp_ratio' | 'fft_lp_ratio_causal' | 'ratio_hp_lp_causal' |  'moving_average' | 'causal_1pole_lp'
...
function sweepr = filter_grid_search(lp,hp,sx,sx_fname,cfg,plot_opts)
    ...
    for r_lp = 1:numel(lp)
        ...
        for r_hp = 1:numel(hp)
            ...
            % run processing
            disp(cfg.(sx_fname).homo_channel);
            sx = run_single_dataset(sx.run_dir, cfg.(sx_fname), sx.channel,verbose)
